#experimento1.py
# EXPERIMENTO 9 : Ensamble con lgb y xgb. Es el 5 pero saco las predicciones del modelo promedio con subsampleo (7 con subsampleo)
import numpy as np
import pandas as pd
...
def lanzar_experimento_9(fecha:str ,semillas:list[int],proceso_ppal:str ="experimento"): #semillas, si queremos hacer una prediccion final, poner solo una semilla en una lista
    n_semillas = len(semillas)
    name=f"{fecha}_EXPERIMENTO_9"
    logger.info(f"PROCESO PRINCIPAL ---> {proceso_ppal}")
    logger.info(f"Comienzo del experimento 9 : {name} con {n_semillas} semillas")
...
## 5. Primer Entrenamiento lgbm con la mejor iteracion y los mejores hiperparametros en [01,02,03] y evaluamos en 04 
    ...
    if proceso_ppal =="experimento":
        ...
        for i,semilla in enumerate(semillas):
            ...
            fecha_name='2025-10-12_00-17-58'
            umbrales_file=f"{fecha_name}_EXPERIMENTO_9.json"
            ...
            fecha_name=fecha_name+'_ensamble_EXPERIMENTO_9'
