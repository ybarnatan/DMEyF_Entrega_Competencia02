#experimento1.py
# EXPERIMENTO 8 : Ensamble con lgb y xgb. Es el 5 pero saco las predicciones del modelo promedio (solo 5 semillas) y subsampleo
import numpy as np
import pandas as pd
...
def lanzar_experimento_8(fecha:str ,semillas:list[int],proceso_ppal:str ="experimento"): #semillas, si queremos hacer una prediccion final, poner solo una semilla en una lista
    n_semillas = len(semillas)
    name=f"{fecha}_EXPERIMENTO_8"
    logger.info(f"PROCESO PRINCIPAL ---> {proceso_ppal}")
    logger.info(f"Comienzo del experimento 8 : {name} con {n_semillas} semillas")
...
#############################################################################-----  DESCOMENTAR  ------############################################
    ...
    f = (df["foto_mes"].isin([MES_01,MES_02,MES_03])) & (df["clase_ternaria"] == "Continua")
    df=df.loc[~f]
    f=df["foto_mes"].isin([MES_01,MES_02,MES_03])
    
    logger.info(f"Se hizo bien el filtrado de continuas en 01 02 03 ? : Los unicos en eso son {df.loc[f , 'clase_ternaria'].unique()}")    
...
## 4. Carga de mejores Hiperparametros
    ...
    bayesiana_fecha_hora_lgbm= '2025-10-11_10-25-49'
    bayesiana_fecha_hora_xgb = '2025-10-11_11-25-33'
...
## 5. Primer Entrenamiento lgbm con la mejor iteracion y los mejores hiperparametros en [01,02,03] y evaluamos en 04 
    ...
    if proceso_ppal =="experimento":
        ...
        for i,semilla in enumerate(semillas):
            ...
            fecha_name=fecha_name+'_ensamble_EXPERIMENTO_8'
