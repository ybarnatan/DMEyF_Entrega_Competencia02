# EXPERIMENTO 10 : Ensamble con lgb y xgb. Es el 5 pero saco las predicciones del modelo promedio con subsampleo (7 con subsampleo)
import numpy as np
import pandas as pd
...
def lanzar_experimento_10(fecha:str ,semillas:list[int],proceso_ppal:str ="experimento"): #semillas, si queremos hacer una prediccion final, poner solo una semilla en una lista
    n_semillas = len(semillas)
    name=f"{fecha}_EXPERIMENTO_10"
    logger.info(f"PROCESO PRINCIPAL ---> {proceso_ppal}")
    logger.info(f"Comienzo del experimento 10 : {name} con {n_semillas} semillas")
...
## 4. Carga de mejores Hiperparametros
    ...
    bayesiana_fecha_hora_lgbm= '2025-10-11_18-16-49'
    bayesiana_fecha_hora_xgb = '2025-10-11_18-16-50'
    ...
    name_best_params_file_xgb=f"best_params_binaria_{bayesiana_fecha_hora_xgb}_xgb.json"
    name_best_iter_file_xgb=f"best_iter_binaria_{bayesiana_fecha_hora_xgb}_xgb.json"

    try:
        with open(bestparams_path+name_best_params_file_lgbm, "r") as f:
    ...
    try:
        with open(bestparams_path+name_best_params_file_xgb, "r") as f:
...
## 5. Primer Entrenamiento lgbm con la mejor iteracion y los mejores hiperparametros en [01,02,03] y evaluamos en 04 
    ...
    if proceso_ppal =="experimento":
        ...
        for i,semilla in enumerate(semillas):
            logger.info(f"Comienzo de la semilla {semilla} del orden {i} de {len(semillas)} semillas  *****************************************")
            

            # Carga del umbral optimo de todas las tiradas : 
            fecha_name='2025-10-12_06-33-54'
            umbrales_file=f"{fecha_name}_EXPERIMENTO_10.json"
            ...
            fecha_name=fecha_name+'_ensamble_EXPERIMENTO_10'
