#experimento1.py
# EXPERIMENTO 6 : Ensamble con lgb y xgb train 02 03
import numpy as np
import pandas as pd
...
def lanzar_experimento_6(fecha:str ,semillas:list[int],proceso_ppal:str ="experimento"): #semillas, si queremos hacer una prediccion final, poner solo una semilla en una lista
    n_semillas = len(semillas)
    name=f"{fecha}_EXPERIMENTO_6"
    logger.info(f"PROCESO PRINCIPAL ---> {proceso_ppal}")
    logger.info(f"Comienzo del experimento 6 : {name} con {n_semillas} semillas")
...
## 5. Primer Entrenamiento lgbm con la mejor iteracion y los mejores hiperparametros en [01,02,03] y evaluamos en 04 
    ...
    if proceso_ppal =="experimento":
        ...
        fecha_name='2025-10-10_18-02-06'
        umbrales_file=f"{fecha_name}_EXPERIMENTO_6.json"
        ...
        fecha_name=fecha_name+'_ensamble_EXPERIMENTO_6'
